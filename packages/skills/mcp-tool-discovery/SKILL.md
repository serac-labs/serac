...
This server registers over 430 `snow_*` tools across 80 domains and lists none of them. A fresh tool list
...
registry, and `adapters` is an empty placeholder. Two more — `fsm` and `predictive-intelligence-MIGRATED`
— are full of tool files the registry never picked up, and that pair is a real bug (see below).
...
## Nine tools you can find in the docs but not on the server

`tools.json` — the manifest the docs site renders — is generated by walking the tool files. The running
server builds its registry from a hard-coded domain map in `shared/tool-registry.ts`. They disagree, which
is tracked as serac-labs/serac#307:

- `tools/fsm/` and `tools/predictive-intelligence-MIGRATED/` are not in that map, so
  `snow_fsm_work_order_manage`, `snow_fsm_dispatch_manage`, `snow_fsm_parts_manage`,
  `snow_create_pi_solution`, `snow_train_pi_solution`, `snow_activate_pi_solution`,
  `snow_list_pi_solutions` and `snow_monitor_pi_training` are published and not present.
- `snow_create_catalog_variable` has a file but `tools/knowledge/index.ts` never re-exports it.

They do not appear in `tool_search` at all, and calling one gives:

```json
{
  "success": false,
  "error": "Tool not found: snow_fsm_work_order_manage",
  "suggestion": "Use tool_search to find available tools",
  "hint": "Use tool_search({query: \"your task\"}) to find the right tool"
}
```

Both the suggestion and the hint are dead ends here — no query reaches a tool that was never registered.

It runs the other way too: `snow_comprehensive_search` is registered and callable but absent from
`tools.json`, because it is a deprecated alias exported under a second name. Prefer
`snow_search_artifacts`.

If a tool you read about in the docs answers "Tool not found", this is why — it is not a session problem
and re-searching will not fix it.
